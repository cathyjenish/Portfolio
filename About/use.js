...
class Mouse {
  ...
  class Rope {
    constructor(config) {
      this.x = config.x
      this.y = config.y
      this.segments = config.segments || 3
      this.gap = config.gap || 5
      this.color = config.color || '#F5F5F5'
  ...
  class App {
    ...
    createRopes() {
      ...
      for (let i = 0; i < half; i++) {
        const x = randomNumBetween(0, App.width * 0.1)  // Place near the left edge
        const y = 0
      ...
      for (let i = 0; i < half + 1; i++) {
        const x = randomNumBetween(App.width * 0.9, App.width)  // Place near the right edge
        const y = 0
        const gap = randomNumBetween(App.height * 0.05, App.height * 0.08)
        const segments = 10
        const rope = new Rope({ x, y, gap, segments })
